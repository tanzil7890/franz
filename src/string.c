  ...
  for (int i = 0; i < length; i++) {
    if (in[i] == '\\') {
      if (in[i + 1] == 'a') res[i - lost] = '\a';
        ...
        // if this escape codes leaks out of bounds, treat \x as just x
        if (i + 3 > length - 1) res[i - lost] = 'x';
        else {
          // get char
          ...
          // we do not allow null chars in franz, if we find one, simply ignore the full escape code
          if (c != '\0') {
            res[i - lost] = c;
          } else {lost += 1;}
