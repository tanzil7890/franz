...
// ie. ./abc/def/../useable.franz and abc/useable.franz both become ./abc/useable.franz.
...
int fileExists(char *path) {
  ...
  if (p_file == NULL) {
    return 0;  // File doesn't exist or not readable
  }
...
// Opens file in append mode and adds contents to end
void appendFile(char *path, char *contents, int lineNumber) {
  ...
  if (p_file == NULL) {
    printf(
      "Runtime Error @ Line %i: Cannot append to file \"%s\".\n",
      lineNumber, path
    );
    exit(0);
  }
